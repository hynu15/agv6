...
class ArmController:
    def __init__(self):
        ...
        self.control_publisher = rospy.Publisher('/robot_agv0/arm_controller/command', 
                                                JointTrajectory, queue_size=10)
        
        # Joint positions
        self.joint_positions = [0.0, 0.0]  # [Link_1_jt, Link_2_jt]
        
        # Joint limits from URDF
        self.joint_limits = {
            'Link_1_jt': {'min': -3.14, 'max': 3.14},  # rotation
            'Link_2_jt': {'min': -1.57, 'max': 0.0}    # up/down
        }
    ...
    def send_joint_command(self):
        ...
        msg.joint_names = ['Link_1_jt', 'Link_2_jt']
    ...
    def rotate_joint_1(self, direction):
        """Rotate joint_1 (positive or negative direction)"""
        increment = self.rotation_increment * direction
        new_value = self.joint_positions[0] + increment
        
        # Apply joint limits
        if new_value <= self.joint_limits['Link_1_jt']['max'] and new_value >= self.joint_limits['Link_1_jt']['min']:
            self.joint_positions[0] = new_value
            return True
        return False

    def move_joint_2(self, direction):
        """Move joint_2 up/down (positive or negative direction)"""
        increment = self.vertical_increment * direction
        new_value = self.joint_positions[1] + increment
        
        # Apply joint limits
        if new_value <= self.joint_limits['Link_2_jt']['max'] and new_value >= self.joint_limits['Link_2_jt']['min']:
    ...
    def keyboard_control(self, stdscr):
        ...
        stdscr.addstr(1, 0, "A/D: Rotate Link_1_jt left/right")
        stdscr.addstr(2, 0, "W/S: Move Link_2_jt up/down")
        ...
        while self.running and not rospy.is_shutdown():
            try:
                ...
                if key != -1:
                    ...
                    # Update display
                    stdscr.addstr(6, 0, f"Link_1_jt (rotation): {self.joint_positions[0]:.2f} rad")
                    stdscr.addstr(7, 0, f"Link_2_jt (vertical): {self.joint_positions[1]:.2f} rad")
